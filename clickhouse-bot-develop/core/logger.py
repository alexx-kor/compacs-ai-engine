#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Structured logging helpers for the RAG pipeline (file + optional console)."""
...
def setup_logging(level: int = logging.INFO) -> logging.Logger:
    """Configure the root logger once for CLI entry points (stderr, lazy %-format friendly)."""
...
class StructuredLogger:
    ...
    def __init__(self, model_tag: str = "llama3.2:3b") -> None:
    ...
    def log_run_end(self) -> None:
        ...
        duration = (
            (datetime.now(timezone.utc) - self._run_start).total_seconds()
            if hasattr(self, "_run_start")
            else 0.0
        )
        self._log("run.end", model_tag=self.model_tag, duration_seconds=duration)
    ...
    def log_rag_checkpoint_a(self, retrieved_chunks: list[Any]) -> None:
    ...
    def log_llm_generate(
        ...
        # Обновляем статистику
        if provider == "ollama":
            if "GPU" in str(self.stats.get('gpu_queries', 0)):
